...
<?php
  ...
  if ($edit) {
    ...
    if ($allFundationGroupsCSV = $FADMIN->getAllFundationGroups()) {
      ...
      foreach (str_getcsv_buckutt($allFundationGroupsCSV, ',') as $group) {
			...
			<?php
			  // On check quel valeur a été posté et on l'affiche
			    if ($allPeriodCSV = $FADMIN->getAllPeriod()) {
			      ...
			      foreach (str_getcsv_buckutt($allPeriodCSV, ',') as $period) {
...
<?php
  ...
  if ($allFundationGroupsCSV = $FADMIN->getAllFundationGroupsLight()) {
  	foreach ($allFundationGroups = str_getcsv_buckutt($allFundationGroupsCSV, ',') as $g => $group) {
  	  // Gestion des ancres
